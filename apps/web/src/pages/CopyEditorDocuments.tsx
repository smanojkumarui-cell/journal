...
export default function CopyEditorDocuments() {
  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      const docs = await instructionDocumentApi.getAll(category, 'CopyEditor');
      setDocuments(docs);
    } catch (error) {
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-light)', marginTop: '0.25rem' }}>Style guides and instruction documents for copy editing</p>
        </div>
      </div>
